...
#		string1.decode('utf-8')
		...
		if(index2 == -1):
			...
			index3 = info[0].find(u'次'.encode('utf-8'), index1)
			if(index3 < index2):
				dltimes = int(info[0][index1: index3])
				index1 = info[0].find(u"正版".encode('utf-8'), index3) + 9
				legal_dltimes = info[0][index1: index2]
				legal_dltimes = int(float(dltimes) * 10000)
			else:
				#get dltimes
				dltimes = info[0][index1: index2]
...
#handle the response of new requests in category
	...
	#game category name
	game_category = hxs.select('//ul[@class = "catelist game"]/li/a/text()').extract()
	#for i in game_category[0:]:
	#	self.printhxs(i)
	#the game category links
	urls = hxs.select('//ul[@class = "catelist game"]/li/a/@href').extract()or['']
